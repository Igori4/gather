...
### Step 2 — Auth-App Project Scaffold [ ]
File: `apps/auth-app/package.json`

- Change `"main"` → `"dist/index.js"`
- Change `"start"` → `"node dist/index.js"`
- Add scripts: `"dev": "nodemon"`, `"build": "tsc -p tsconfig.json"`, `"typecheck": "tsc --noEmit"`
- Add scripts: `"db:generate": "dotenv -e ../../.env.local -e ../../.env -- prisma generate"`, `"db:migrate": "dotenv -e ../../.env.local -e ../../.env -- prisma migrate dev"`
- Add runtime deps: `express bcrypt cookie-parser cors helmet jsonwebtoken resend zod @prisma/client @prisma/adapter-pg pg`
- Add dev deps: `typescript ts-node nodemon prisma dotenv-cli @types/express @types/bcrypt @types/cookie-parser @types/cors @types/jsonwebtoken @types/node ts-jest`
- Keep Mate Academy tooling (`eslint`, `jest`, `prettier`, `mate-scripts`)
